...
core_columns = ['_RFHLTH', '_TOTINDA', '_SMOKGRP', '_DRNKWK2', '_MICHD', '_LTASTH1',
...
########handle outliers - generated by program#######
# Calculate the z-scores of AlcoholIntake
z_scores = (data_filtered['AlcoholIntake'] - data_filtered['AlcoholIntake'].mean()) / data_filtered['AlcoholIntake'].std()
# cook's distance
from statsmodels.stats.outliers_influence import OLSInfluence
influence = OLSInfluence(data_filtered['AlcoholIntake'])
cooks_d = influence.cooks_distance
# Identify the rows with large cook's distance values
cooks_mask = cooks_d > 4 / data_filtered.shape[0]
# Remove the rows with large cook's distance values
data_filtered = data_filtered[~cooks_mask]
# Calculate the z-scores of AlcoholIntake again
z_scores = (data_filtered['AlcoholIntake'] - data_filtered['AlcoholIntake'].mean()) / data_filtered['AlcoholIntake'].std()
